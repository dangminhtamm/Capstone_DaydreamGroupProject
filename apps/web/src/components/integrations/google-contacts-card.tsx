...
export function GoogleContactsCard() {
  ...
  const {
    ...
    loadContacts,
    clearFeedback,
  } = useGoogleContactsIntegration(auth);
  ...
  if (!isAuthenticated) {
    return (
      <section className="rounded-3xl border border-slate-200/80 bg-gradient-to-br from-white via-white to-fuchsia-50/30 p-6 shadow-sm shadow-slate-200/60 dark:border-slate-700 dark:from-slate-800 dark:via-slate-800 dark:to-fuchsia-950/20 dark:shadow-slate-900/40">
        <div className="mb-5">
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center dark:border-slate-600">
          <p className="text-sm font-medium text-slate-900 dark:text-slate-200">
            Sign in to sync Google Contacts.
          </p>
        </div>
      </section>
    );
      ...
      <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                {isLoading ? 'Checking contacts...' : status?.connected ? 'Google connected' : 'Reconnect Google to add Contacts scope'}
              </p>
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                {status
                  ? `${status.contactCount} contact${status.contactCount === 1 ? '' : 's'} saved${status.lastSyncedAt ? `, last synced ${new Date(status.lastSyncedAt).toLocaleDateString()}` : ''}`
                  : 'Contacts use the same Google OAuth connection as Calendar.'}
              </p>
            </div>
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="cursor-pointer rounded-xl border border-fuchsia-200 bg-fuchsia-50 px-4 py-2.5 text-sm font-semibold text-fuchsia-700 transition hover:bg-fuchsia-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-fuchsia-800 dark:bg-fuchsia-950/40 dark:text-fuchsia-200 dark:hover:bg-fuchsia-900/50"
          >
            {isConnecting ? 'Connecting...' : 'Reconnect Google'}
          </button>
          <button
            type="button"
            onClick={() => void syncGoogleContacts(50)}
            disabled={!status?.connected || isSyncing}
            className="cursor-pointer rounded-xl bg-fuchsia-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-fuchsia-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
            ...
            {contacts.slice(0, 5).map((contact) => (
              <div key={contact.id} className="px-4 py-3">
                ...
                <p className="mt-1 truncate text-xs text-slate-500 dark:text-slate-400">
                  {[contact.emailAddresses[0], contact.organizations[0]].filter(Boolean).join(' · ') || 'No email or organization saved'}
                </p>
              </div>
            ))}
          </div>
